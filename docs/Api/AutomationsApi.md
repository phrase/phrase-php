...
> automationTrigger($account_id, $id, $x_phrase_app_otp)
...
try {
    $apiInstance->automationTrigger($account_id, $id, $x_phrase_app_otp);
} catch (Exception $e) {
    echo 'Exception when calling AutomationsApi->automationTrigger: ', $e->getMessage(), PHP_EOL;
}
?>
```
